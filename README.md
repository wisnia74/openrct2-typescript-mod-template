...
- if you have some existing files in OpenRCT2 `plugin` folder, but you've decided to remove them from `./src` and then compiled files, files removed from OpenRCT2 `plugin` folder will still be there, so you have to remove them manually
